...
begin
  ...
  try
    ...
    xnTimers.Start(0);
    sgd.sort;
    memo_2.Lines.add('ok count = ' + IntToStr(gd.RowCount));
    // memo_2.Lines.add('array = ' + gd.AsDebug);

    for n := 0 to sgd.RowCount - 1 do
    begin
      a := BuildArray(2 * n);
      s := sgd.Seek1(a);
    ...
    memo_2.Lines.add('ok count = ' + IntToStr(sgd.RowCount));
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if a[0] < sgd.ValueFloat(0, sgd.RowCount - 1) then
